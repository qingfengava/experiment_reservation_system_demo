...
bool FileService ::loadStudents(const QString &filePath) {
  ...
  QString load_file_path = QDir::fromNativeSeparators(
      LOAD_PATH + filePath); // 拼接路径，处理路径分隔符
...
bool FileService ::loadExperiments(const QString &filePath) {
  ...
  while (!in.atEnd()) {
    ...
    if (!StartTime.isValid() || !EndTime.isValid()) {
      qDebug() << "时间解析失败，跳过该行：" << line << "| 开始时间："
               << lines[2] << "| 结束时间：" << lines[3];
      continue;
    }
    ...
    if (!totalSeatsOk || !reservedCountOk) {
      qDebug() << "数字转换失败，跳过该行：" << line << "| 总席位：" << lines[4]
               << "| 已预约：" << lines[5];
      continue;
    }
    ...
    if (publishStr == "true" || publishStr == "1") {
      ...
      qDebug() << "发布状态格式错误，跳过该行：" << line << "| 状态："
               << lines[6];
      continue;
    }

    // ========== 构造Experiment对象并加入容器 ==========
    g_experiments.emplace_back(expId,         // 0: 实验编号
                               expName,       // 1: 实验名称
                               StartTime,     // 2: 开始时间
                               EndTime,       // 3: 结束时间
                               totalSeats,    // 4: 总席位
...
bool FileService ::loadReservations(const QString &filePath) {
  ...
  while (!in.atEnd()) {
    ...
    if (!scoreOk) {
      qDebug() << "数字转换失败，跳过该行：" << line << "| 成绩：" << lines[6];
      continue;
    }

    g_reservations.emplace_back(lines[0], lines[1], lines[2], lines[3],
                                lines[4], lines[5], Score);
  }

  file_in.close();
  qDebug() << "loadReservations成功,共读取" << g_reservations.size()
           << "条账户";
...
bool FileService ::loadAll(void) {
  if (FileService ::loadStudents("students.txt") &&
      FileService ::loadTeachers("teachers.txt") &&
      FileService ::loadExperiments("experiments.txt") &&
      FileService ::loadReservations("reservations.txt"))
    return true;
...
bool FileService ::saveStudents(const QString &filePath) {
  ...
  for (auto index : g_students) {
    out << index.studentId << "," << index.name << "," << index.college << ","
        << index.major << "," << index.password << "\n";
  }
...
bool FileService ::saveTeachers(const QString &filePath) {
  ...
  for (auto index : g_teachers) {
    out << index.teacherId << "," << index.name << "," << index.college << ","
        << index.password << "\n";
  }
...
bool FileService ::saveExperiments(const QString &filePath) {
  ...
  out << "expId,expName,startTime,endTime,totalSeats,reservedCount,published,"
  ...
  for (auto index : g_experiments) {
    ...
    if (!index.startTime.isValid()) {
      qDebug()
          << "[转换失败] startTime对应的QDateTime对象无效(未初始化/解析错误)";
      StartTime = "无效时间";
    }
    if (!index.endTime.isValid()) {
      qDebug()
          << "[转换失败] endTime对应的QDateTime对象无效(未初始化/解析错误)";
      EndTime = "无效时间";
    }
    ...
    out << index.expId << "," << index.expName << "," << StartTime << ","
        << EndTime << "," << TotalSeats << "," << ReservedCount << ","
        << Published << "," << index.teacherId << "," << index.teacherName
        << "\n";
  }
...
bool FileService ::saveReservations(const QString &filePath) {
  ...
  for (auto index : g_reservations) {
    ...
    out << index.expId << "," << index.expName << "," << index.studentId << ","
        << index.studentName << "," << index.teacherId << ","
        << index.teacherName << "," << Score << "\n";
  }
...
bool FileService ::saveAll(void) {
  if (FileService ::saveStudents("students.txt") &&
      FileService ::saveTeachers("teachers.txt") &&
      FileService ::saveExperiments("experiments.txt") &&
      FileService ::saveReservations("reservations.txt"))
    return true;
...
bool FileService ::exportExperimentsToCSV(const QString &filePath) {
  ...
  qDebug() << "exportExperimentsToCSV成功写入" << g_experiments.size()
           << "条数据";
...
bool FileService ::exportReservationsToCSV(const QString &filePath) {
  ...
  for (auto index : g_reservations) {
    ...
    out << index.expId << "," << index.expName << "," << index.studentId << ","
        << index.studentName << index.teacherId << "," << index.teacherName
        << "," << Score << "\n";
  }

  file_out.close();

  qDebug() << "exportReservationsToCSV成功写入" << g_reservations.size()
           << "条数据";
...
bool FileService ::exportExperimentsToTXT(const QString &filePath) {
  ...
  qDebug() << "exportExperimentsToTXT成功写入" << g_experiments.size()
           << "条数据";
...
bool FileService ::exportReservationsToTXT(const QString &filePath) {
  ...
  qDebug() << "exportReservationsToTXT成功写入" << g_reservations.size()
           << "条数据";
